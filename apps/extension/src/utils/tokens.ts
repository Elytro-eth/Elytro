...
export const fetchTokens = async (chainId: number, maxCount?: number): Promise<TTokenInfo[]> => {
  try {
...
export const getTop100TokenAddresses = async (chainId: number): Promise<TTokenInfo[]> => {
  ...
  try {
    ...
    const tokens = await fetchTokens(chainId); // all tokens
    ...
    console.error('Elytro: Failed to get top 100 tokens:', error instanceof Error ? error.message : 'Unknown error');
    return [];
  }
};

export const updateUserImportedTokens = async (chainId: number, token: TTokenInfo): Promise<void> => {
  const storageKey = CONFIG.CACHE.KEYS.USER_TOKENS(chainId);

  try {
    const prevTokens = ((await localStorage.get(storageKey)) as TTokenInfo[]) || [];
    ...
    console.error('Elytro: Failed to update user tokens:', error instanceof Error ? error.message : 'Unknown error');
  }
};
...
export const getTokenList = async (chainId: number): Promise<TTokenInfo[]> => {
  try {
    const [userTokens, top100Tokens] = await Promise.all([
      localStorage.get(CONFIG.CACHE.KEYS.USER_TOKENS(chainId)) as Promise<TTokenInfo[]>,
      getTop100TokenAddresses(chainId),
    ]);

    const userTokenAddresses = new Set((userTokens || []).map((token) => token.address.toLowerCase()));

    const filteredTop100Tokens = top100Tokens.filter((token) => !userTokenAddresses.has(token.address.toLowerCase()));

    return [...(userTokens || []), ...filteredTop100Tokens];
  } catch (error) {
    console.error('Elytro: Failed to get token list:', error instanceof Error ? error.message : 'Unknown error');
    return [];
  }
};
